...
use Symfony\Config\Doctrine\Dbal\ConnectionConfig\SlaveConfig;
...
class ConnectionConfig
{
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('slaves', $value)) {
            ...
            $this->slaves = array_map(function ($v) {
                return is_array($v) ? new SlaveConfig($v) : $v;
            }, $value['slaves']);
            unset($value['slaves']);
        }
        ...
        if (array_key_exists('replicas', $value)) {
            ...
            $this->replicas = array_map(function ($v) {
                return is_array($v) ? new ReplicaConfig($v) : $v;
            }, $value['replicas']);
     ...
     */
    public function slave(string $name, mixed $value = []): SlaveConfig|static
    {
        if (!is_array($value)) {
            $this->_usedProperties['slaves'] = true;
        ...
        if (!isset($this->slaves[$name]) || !$this->slaves[$name] instanceof SlaveConfig) {
            $this->_usedProperties['slaves'] = true;
            $this->slaves[$name] = new SlaveConfig($value);
    ...
    public function replica(string $name, mixed $value = []): ReplicaConfig|static
    {
        ...
        if (!isset($this->replicas[$name]) || !$this->replicas[$name] instanceof ReplicaConfig) {
            $this->_usedProperties['replicas'] = true;
            $this->replicas[$name] = new ReplicaConfig($value);
        } elseif (1 < func_num_args()) {
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['slaves'])) {
            $output['slaves'] = array_map(function ($v) {
                return $v instanceof SlaveConfig ? $v->toArray() : $v;
            }, $this->slaves);
        }
        if (isset($this->_usedProperties['replicas'])) {
            $output['replicas'] = array_map(function ($v) {
                return $v instanceof ReplicaConfig ? $v->toArray() : $v;
            }, $this->replicas);
        }
